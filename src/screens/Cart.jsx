    ...
    <View>
      <View className="fixed px-4">
        ...
        <TouchableOpacity onPress={() => navigation.navigate('CustomerHome')}>
          <Text className="self-end text-[#e79951] py-2">
            continue shopping
          </Text>
        </TouchableOpacity>
        {cart.length > 0 && (
          <Text className="bg-[#d9d9d9] text-center p-3">
            Proceed to payments
          </Text>
        )}
      </View>
        ...
        <ScrollView className="grid grid-row-1 h-[82%] space-y-1">
          {cart.length > 0 ? (
            cart?.map((i, index) => (
              <View
                key={index}
                className="flex-row gap-2 border-y-[.5px] border-gray-200">
                ...
                <View className="flex-row flex-1 pr-5 justify-end  items-center gap-x-5">
                  <TouchableOpacity
                    className="p-3 "
                    onPress={() =>
                      navigation.navigate('ProductDetail', {orderItem: i})
                    }>
                    <Text>
                      <FontAwesomeIcon icon={faPencil} />
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => remove_from_cart(i.id)}>
            ...
            <View className="items-center">
              <View
                className="bg-[#bedbfe] h-48 w-48 items-center justify-center"
                style={{borderRadius: 100}}>
                <Image
                  source={require('../assets/products/cart.png')}
                  className="w-28 h-28 "
                />
              </View>
